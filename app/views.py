...
from django.contrib import messages  # this builtin message framework of django which give a automatic conformation when registration is done  
from .models import Customer, Product, Cart, OrderPlaced  #imported all model from models.py
from .forms import CustomerRegistrationForm, CustomerProfileForm #import registration and login form
...
def add_to_cart(request):
	user = request.user # esse hame current user mil jata hai 
	item_already_in_cart1 = False
	product = request.GET.get('prod_id') # yaha per hame product ki id mmile ge add to kard karne ke bad 
...
def mobile(request, data=None):   #time stamp 02:05
	totalitem = 0
	if request.user.is_authenticated:
		totalitem = len(Cart.objects.filter(user=request.user))
	if data==None :
...
class ProfileView(View):
	...
	def post(self, request):
		...
		if form.is_valid():
			...
			#below two line code me hame above data ko save karva rahe he data base me 
